(() => {
    ...
    function pageNavigation() {
        ...
        if (getHash()) {
            ...
            if (goToHash && modules_flsModules.fullpage) {
            ...
            } else goToHash ? gotoblock_gotoBlock(goToHash, true, 500, 20) : null;
        }
    }
    setTimeout((() => {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        if (splitTextBoth || splitTextLines) updateIndexes();
        ...
        let data = {};
        fetch("files/data.json").then((response => response.json())).then((json => data = json)).catch((error => console.error("Error fetching data:", error)));
        if (sections.length) {
            ...
            sections.forEach(((section, index) => {
                ...
                button.addEventListener("click", (() => {
                    if (button.classList.contains("_next")) {
                        ...
                        currentSection++;
                        if (currentSection < sections.length) sections[currentSection].classList.add("_active"); else {
        ...
        function formSubmit() {
            ...
            function formSubmitAction(form) {
                ...
                let selectedGroup;
                if (groupA > groupB && groupA > groupC) selectedGroup = "groupA"; else if (groupB > groupA && groupB > groupC) selectedGroup = "groupB"; else if (groupC > groupA && groupC > groupB) selectedGroup = "groupC"; else {
                    const groups = [ "groupA", "groupB", "groupC" ];
                    selectedGroup = groups[Math.floor(Math.random() * groups.length)];
                }
                console.log(`Selected Group: ${selectedGroup}`);
                updateResult(selectedGroup);
        ...
        function updateResult(group) {
            ...
            } else console.error("Group data not found");
        }
